...
def process():
    ...
    if not filename or not subdir:
        return Response(
            json.dumps({"status": "error", "message": "Campos 'filename' e 'subdir' são obrigatórios."}, ensure_ascii=False),
    ...
    Thread(target=background_task, args=({"filename": filename, "subdir": subdir},)).start()
